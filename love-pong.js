// Love Pong V1 - low-latency Broadcast sync + 60fps rendering
...
function ensurePongFastChannel(){
 ...
 pongFastChannel=db.channel(topic,{config:{broadcast:{self:false}}})
   .on('broadcast',{event:'pong_state'},({payload})=>{
      ...
      pongTarget={...payload,guestY:pongLocalPaddle};
      ...
      $('pongRematchBtn').classList.toggle('hidden',!payload.winner||!isHost);
   })
...
async function startLovePong(){
 ...
 const state=freshPong();pongRemoteGuestY=.5;pongLastPersistScore='0-0';
 const {data,error}=await db.from('arcade_rooms').update({selected_game:'love-pong',status:'playing',game_state:state,host_paddle:.5,guest_paddle:.5}).eq('id',currentRoom.id).select().single();
...
function showLovePong(room){
 ...
 $('pongRoomCode').textContent=room.room_code;$('pongBabyName').textContent=room.host_name||'Baby';$('pongSayangName').textContent=room.guest_name||'Sayang';
 $('pongScore').textContent=`${s.hostScore||0} — ${s.guestScore||0}`;
 $('pongStatus').textContent=s.winner?`${s.winner==='host'?(room.host_name||'Baby'):(room.guest_name||'Sayang')} wins! 🏆❤️`:(s.serving?'Get ready...':'First to 3 wins ❤️');
 pongLocalPaddle=isHost?(room.host_paddle??.5):(room.guest_paddle??.5);pongRemoteGuestY=room.guest_paddle??pongRemoteGuestY;
 ensurePongFastChannel();
 const drawState={...s,hostY:room.host_paddle??s.hostY,guestY:room.guest_paddle??s.guestY};
 $('pongRematchBtn').classList.toggle('hidden',!s.winner||!isHost);
 if(isHost){currentRoom.game_state={...s};drawPong(drawState);if(!pongLoop)startPongHostLoop();stopGuestAnimation()}
 else{pongTarget=drawState;if(!pongVisual)pongVisual={...drawState};startGuestAnimation();if(pongLoop){clearInterval(pongLoop);pongLoop=null}}
}

function drawPong(s){
 const c=$('pongCanvas'),ctx=c.getContext('2d'),w=c.width,h=c.height;ctx.clearRect(0,0,w,h);
 ctx.fillStyle='rgba(255,255,255,.05)';ctx.fillRect(0,0,w,h);ctx.strokeStyle='rgba(255,255,255,.14)';ctx.setLineDash([8,8]);ctx.beginPath();ctx.moveTo(w/2,0);ctx.lineTo(w/2,h);ctx.stroke();ctx.setLineDash([]);
 const ph=h*PADDLE_H,pw=10;ctx.fillStyle='#f2cbd2';ctx.fillRect(12,(s.hostY??.5)*h-ph/2,pw,ph);ctx.fillStyle='#e8dfff';ctx.fillRect(w-22,(s.guestY??.5)*h-ph/2,pw,ph);
...
function startGuestAnimation(){
 ...
 const tick=()=>{
   ...
   pongVisual.ballX=lerp(pongVisual.ballX??.5,pongTarget.ballX??.5,.48);
   pongVisual.ballY=lerp(pongVisual.ballY??.5,pongTarget.ballY??.5,.48);
   pongVisual.hostY=lerp(pongVisual.hostY??.5,pongTarget.hostY??.5,.5);
   pongVisual.guestY=pongLocalPaddle;
   pongVisual.hostScore=pongTarget.hostScore;pongVisual.guestScore=pongTarget.guestScore;pongVisual.winner=pongTarget.winner;
...
function startPongHostLoop(){
 ...
 pongLoop=setInterval(async()=>{
   ...
   const s=currentRoom.game_state;if(s.winner){broadcastPongState(s,true);return}
   s.hostY=currentRoom.host_paddle??s.hostY;s.guestY=pongRemoteGuestY??s.guestY;
   if(Date.now()>=(s.serveAt||0)){
     s.serving=false;s.ballX+=s.vx;s.ballY+=s.vy;
     if(s.ballY<BALL_R){s.ballY=BALL_R;s.vy=Math.abs(s.vy)}
     if(s.ballY>1-BALL_R){s.ballY=1-BALL_R;s.vy=-Math.abs(s.vy)}
     const paddleHit=side=>Math.abs(s.ballY-(side==='host'?s.hostY:s.guestY))<PADDLE_H/2+.035;
     ...
     if(s.hostScore>=WIN_SCORE){s.winner='host';s.serving=false}
     if(s.guestScore>=WIN_SCORE){s.winner='guest';s.serving=false}
   }
   drawPong(s);$('pongScore').textContent=`${s.hostScore||0} — ${s.guestScore||0}`;
   $('pongStatus').textContent=s.winner?`${s.winner==='host'?(currentRoom.host_name||'Baby'):(currentRoom.guest_name||'Sayang')} wins! 🏆❤️`:(s.serving?'Get ready...':'First to 3 wins ❤️');
   broadcastPongState(s,false);
   const scoreKey=`${s.hostScore}-${s.guestScore}`;
   if(scoreKey!==pongLastPersistScore||s.winner||Date.now()-pongLastPersist>1200){pongLastPersistScore=scoreKey;pongLastPersist=Date.now();const snap={...s};db.from('arcade_rooms').update({game_state:snap}).eq('id',currentRoom.id).then(({data,error})=>{});}
 },16);
}

function broadcastPongState(s,force){
 const now=Date.now();if(!pongFastChannel||(!force&&now-pongLastBroadcast<32))return;pongLastBroadcast=now;
 pongFastChannel.send({type:'broadcast',event:'pong_state',payload:{phase:'pong',hostY:s.hostY,guestY:s.guestY,ballX:s.ballX,ballY:s.ballY,hostScore:s.hostScore,guestScore:s.guestScore,winner:s.winner,serving:s.serving}});
}
...
async function setPongPaddle(norm){
 ...
 if(isHost){currentRoom.host_paddle=norm;currentRoom.game_state.hostY=norm;drawPong({...currentRoom.game_state,guestY:pongRemoteGuestY})}
 else{currentRoom.guest_paddle=norm;if(pongVisual)pongVisual.guestY=norm;ensurePongFastChannel();pongFastChannel?.send({type:'broadcast',event:'guest_paddle',payload:{y:norm}})}
 const now=Date.now();if(now-(setPongPaddle.last||0)<180)return;setPongPaddle.last=now;
 const patch=isHost?{host_paddle:norm}:{guest_paddle:norm};db.from('arcade_rooms').update(patch).eq('id',currentRoom.id).then(()=>{});
}
